  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        multiline
        minRows={4}
        onChange={(e)=>{handleNewAddress({isAddingNewAddress:true,value:e.target.value})}}
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setAddresses({all: response.data ,isAddingNewAddress:false});
      return response.data;
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
  ...
  const performCheckout = async (token, items, addresses) => {
    if(validateRequest(items,addresses)){
      try{
        let res=await axios.post(`${config.endpoint}/cart/checkout`,{'addressId':addresses.selected},{headers: {
          Authorization: `Bearer ${token}`,
        },})
        if(res.data){
          let wallet=localStorage.getItem('balance');
          let remainder=wallet- getTotalCartValue(items);
          localStorage.setItem('balance',remainder);
          history.push('/thanks');
        }
      }
      catch(e){
